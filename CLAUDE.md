...
- **Caddyfile.example ist ebenfalls ungetestet.** Zum Prüfen lokal nachbauen: Domain → `:PORT`, `auto_https off`, `reverse_proxy` → `respond`, dann per curl gegen einen Ordner mit Fremd-Datei (`design.json`, `dump.sql`) fahren — die müssen 404 geben, `/`, `*.html`, `*.css`, Unterordner-Assets weiterhin 200.
